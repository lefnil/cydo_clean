...
export default function LYDCReports() {
  const { user } = useAuth();
  ...
  const fetchReports = async () => {
    setError(null);
    try {
      const data = await fetchFromGAS('get_lydc_reports', { user_id: user?.id, role: user?.role }).catch(err => {
        console.error('LYDC Reports API Error:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await fetchFromGAS('create_lydc_report', { 
        user_id: user?.id,
  ...
  const handleApprove = async (reportId: string) => {
    try {
      await fetchFromGAS('update_lydc_report_status', {
        id: reportId,
        status: 'approved'
      });
      fetchReports();
  ...
  const COLORS = ['#81d2ad', '#f59e0b', '#ef4444', '#3b82f6'];
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-jewel">LYDC Reports</h1>
          <p className="text-jewel/70 mt-1">Manage and view Local Youth Development Council submissions.</p>
        </div>
      </div>
      ...
      {error && (
        <div className="glass rounded-3xl p-6 bg-red-50/50 border border-red-200">
          ...
          <button 
            ...
            className="mt-3 px-4 py-2 bg-vista text-white rounded-xl hover:bg-vista/90 transition-colors text-sm"
          >
            Retry
          </button>
        </div>
      ...
      <div className="flex gap-3">
        {canApprove && (
          <>
            <button
              onClick={() => exportToPDF(reports, 'lydc_reports')}
            ...
            <button
              ...
              className="px-4 py-2 bg-jewel font-semibold text-white rounded-xl shadow-lg shadow-vista/30 hover:bg-vista/90 transition-all flex items-center gap-2"
            >
        ...
        {!canApprove && (
          <button
            onClick={() => setIsModalOpen(true)}
            className="px-4 py-2 bg-jewel text-white rounded-xl shadow-lg shadow-vista/30 hover:bg-vista/90 transition-all flex items-center gap-2"
          >
            <Plus size={20} /> New Report
          </button>
        )}
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-white/50 border border-white/30 rounded-xl focus:ring-2 focus:ring-vista focus:border-transparent outline-none transition-all placeholder-jewel/40 text-jewel"
                />
              </div>
            </div>
                ...
                <tr key={report.id} className="border-b border-jewel/5 hover:bg-white/30 transition-colors">
                  <td className="py-4 px-4 font-medium text-jewel">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-vista/10 rounded-lg text-vista">
                        <FileText size={16} />
                      </div>
                      {report.title}
                    </div>
                    ...
                    {canApprove && report.status !== 'approved' ? (
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button 
                          ...
                          className="text-vista hover:text-vista/70 font-medium text-sm"
                        >
                          View
                        </button>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
          ...
          >
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-xl font-bold text-vista">Submit New LYDC Report</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-vista focus:border-transparent outline-none"
                  placeholder="e.g., Annual Action Plan"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-vista focus:border-transparent outline-none resize-none"
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-6 py-2 bg-vista text-white rounded-xl shadow-lg shadow-vista/30 hover:bg-vista/90 transition-all disabled:opacity-70"
                >
      ...
      {selectedReport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
          ...
          >
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-xl font-bold text-vista">Report Details</h2>
            ...
            <div className="p-6 border-t border-gray-100 flex justify-end">
              <button
                ...
                className="px-4 py-2 bg-vista text-white rounded-xl hover:bg-vista/90 transition-colors"
              >
                Close
              </button>
            </div>
